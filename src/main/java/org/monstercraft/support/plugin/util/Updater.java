		...
		try {
			...
			if (firstNode.getNodeType() == 1) {
				...
				if (cur.contains(version)) {
					Configuration.log("You are using the latest version of MonsterTickets");
				} else {
					Configuration.log(cur + " is out! You are running " + version);
					Configuration.log("Update MonsterMOTD at: http://dev.bukkit.org/server-mods/monstermotd");
				}
			}
